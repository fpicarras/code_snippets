/**
 * @file client.c
 * @author Filipe Piçarra (git: fpicarras)
 * @brief This is an example of how to run a UDP client, in this case we just send strings to the
 * server from the keyboard... And the server echoes back! [See UDP Server Example]
 * 
 * @version 0.1
 * @date 2024-02-16
 */

//Compile:
//gcc -o client UDPclient.c sockets.c -Wall

#include "sockets.h"

int main(){
    char buffer[BUFFER_SIZE+1];
    //Create a socket that connects to a machine with a certain IP on a specified port
    Socket *sock = UDPSocket("0.0.0.0", "2345");

    //We signal the other server to close if we send "end", in this case, the server will return "Server Closing"
    while(fgets(buffer, BUFFER_SIZE, stdin)){
        Send(sock, buffer);
        Recieve(sock, buffer);
        printf("Recieved: %s\n", buffer);

        if(strcmp(buffer, "Server Closing")==0) break;
    }

    //Close the socket
    closeSocket(sock);

    return 0;
}